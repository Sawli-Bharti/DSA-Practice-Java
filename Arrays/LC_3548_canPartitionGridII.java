/*
    ...
    Brute force: check every partion to see if we can remove element in case of different sum partitions
    TC:- O(n*m*(n+m))
    Sc:- O(1)
    ...
    ->first compute total sum, and store all elements in bottom map and right map
    ->then we try all the possible cuts
    ->in horizontal row, when move to next cut, we shift the row from bottom to top map
    ->Update:
    top sum, bottom map and top map
    then check if top sum == bottom sum
    if yes return true
    else diff = abs(top sum - bottom sum)
    ->now check which map has more sum and then try to remove diff from that map
    ->make sure connectivity do not get affected when you remove an element
    cases when connectivity is affected
    if we have 1d arrays on one side of cut or we have only 1 element
